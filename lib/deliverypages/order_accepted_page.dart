...
class _AvailableOrdersPageState extends State<AvailableOrdersPage> {
        ...
        if (data['accept'] == false &&
          ...
          if (distanceInKm <= 5) {
            nearbyOrders.add({
              'docId': doc.id,
              'orderId': data['orid'],
              'restaurantName': data['restname'],  // Now using Firestore data
              'orderName': data['orname'],  // Now using Firestore data
  ...
  Future<void> _acceptOrder(String docId) async {
    try {
      ...
      var orderDoc = await docRef.get();
      var orderData = orderDoc.data() as Map<String, dynamic>;

      // Pass dynamic data from Firestore to ActiveDeliveryPage
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => ActiveDeliveryPage(
            orderId: orderData['orid'],
            restaurantName: orderData['restname'],
            orderName: orderData['orname'],
            orderLat: orderData['source'].latitude,
            orderLng: orderData['source'].longitude,
